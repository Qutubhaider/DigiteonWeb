...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getEnrollData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getEnrollData(lsPageNo, fsSortColumn, "asc");
    }
}

function getEnrollData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();   
    var liSize = $('#ddPageSize').val();

    var loData = new Object();
    var pg = fsPageNo;
    loData.TrainingId = $('#unTrainingId').val();
    loData.sort_column = fsSortColumn;
    loData.sort_order = fsSortDirection;
    loData.pg = pg;
    loData.size = liSize;

    loadMyRequest(msGetEnrollDataList, "GET", loData, getEnrollDataSuccess, getEnrollDataError);
    $('#preloader').hide();
}

function getEnrollDataError(foResponse) {
    hideLoading();
}

function getEnrollDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#EnrollList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#EnrollList').find("table tbody").empty();
        $('#EnrollList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getEnrollData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
