...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getCareerApplicationData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getCareerApplicationData(lsPageNo, fsSortColumn, "asc");
    }
}

function getCareerApplicationData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var liSize = $('#ddPageSize').val();
    var lsApplicantName = $('#txtApplicantName').val();
    var liCareerId = $('#hdnInCareerId').val();
    var loData = new Object();
    var pg = fsPageNo;
    loData.applicantName = lsApplicantName; 
    loData.inCareerId = liCareerId;
    loData.sort_column = fsSortColumn;
    loData.sort_order = fsSortDirection;
    loData.pg = pg;
    loData.size = liSize;

    loadMyRequest(msGetCareerApplicationDataList, "GET", loData, getCareerApplicationDataSuccess, getCareerApplicationDataError);
    $('#preloader').hide();
}

function getCareerApplicationDataError(foResponse) {
    hideLoading();
}

function getCareerApplicationDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#CareerApplicationList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#CareerApplicationList').find("table tbody").empty();
        $('#CareerApplicationList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getCareerApplicationData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
