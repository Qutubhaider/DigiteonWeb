...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getCareerData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getCareerData(lsPageNo, fsSortColumn, "asc");
    }
}

function getCareerData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var liSize = $('#ddPageSize').val();
    var lsJobTitle = $('#txtJobTitle').val();
    var loData = new Object();
    var pg = fsPageNo;
    loData.jobTitle = lsJobTitle;
    loData.sort_column = fsSortColumn;
    loData.sort_order = fsSortDirection;
    loData.pg = pg;
    loData.size = liSize;

    loadMyRequest(msGetCareerDataList, "GET", loData, getCareerDataSuccess, getCareerDataError);
    $('#preloader').hide();
}

function getCareerDataError(foResponse) {
    hideLoading();
}

function getCareerDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#CareerList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#CareerList').find("table tbody").empty();
        $('#CareerList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getCareerData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
